...
DB_FILE = r"C:\Users\jjunh\OneDrive\바탕 화면\대학\4학년 2학기\데이터베이스\데.베 구축\project2.db"
    ...
    for _, row in valid_df.iloc[::-1].iterrows():
        ...
        if coords: 
            ...
            if ratio < 0.33:
                ...
                # Red: 0 -> 255
                local_ratio = ratio * 3
                r = int(0 + 255 * local_ratio) # 점점 노랑으로
                g = 255
                b = 0
            elif ratio < 0.66:
                # 중간 (연두 ~ 노랑 ~ 주황)
                ...
                r = 255
                g = int(255 - (255 * local_ratio * 0.5)) # 약간 주황빛으로
                b = 0
            else:
                # 많음 (주황 ~ 빨강)
                local_ratio = (ratio - 0.66) * 3
                r = 255
                g = int(128 - (128 * local_ratio)) # 주황에서 완전 빨강으로
                b = 0

            # 간단한 신호등 색상 로직 (더 확실한 구분)
            if count <= 5:
...
if analysis_button and len(date_range) == 2:
    ...
    with kpi_container:
        ...
        kpi1.metric(label="총 매물 수", value=f"{total_count:,.0f} 건")
        kpi2.metric(label="평균 가격", value=f"{avg_price:,.0f} 원")
        with kpi3: st.metric(label="가격 변동 (7일)", value="0.0 %"); st.caption("개발 중")
        ...
        with kpi4: 
            st.metric(label="최다 거래 지역", value=most_frequent_region)
            if most_frequent_region != "-": st.caption(f"총 {most_frequent_count}건")
